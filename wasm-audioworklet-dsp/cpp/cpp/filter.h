...
class Filter {
public:
  Filter(FilterType type, float frequency, float Q, float gain,
         float sample_rate)
      : type(type), frequency(frequency), q(Q), gain(gain),
  ...
  void process(float input, float *output) { biquad.process(input, output); }
